...
class TeacherAdmissionNews extends ComponentBase
{
    ...
    {
        return [
            'name' => 'TeacherAdmissionNews',
            'description' => 'Müellim qebulu kategoryasına aid olan kayıtları listeler'
        ];
    }
    ...
    protected function listRecords()
    {
        ...
        $model = new  \Sabah\News\Models\Category();
        $model = $model->where('id', 9)->first();
        $model = $model->news();
        ...
        $records = $model = $this->paginate($model);
        return $records;
    }
    ...
    protected function skip($model)
    {
        ...
        if (strlen($recordsPerPage)) {
            ...
            $news = \Sabah\News\Models\News::orderBy($sortColumn, $sortDirection)->take($skip)->get();
            $array = $news->lists('id');
